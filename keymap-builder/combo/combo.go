...
type Combo struct {
	...
	RCs    []rowcol.T // array   A list of key position indices for the keys which should trigger the combo
...
func (c Combo) RenderKeys() string {
	return strings.Join(Map(c.RCs, func(x rowcol.T) string {
		return x.Render()
	}), " ")
}
...
func Add(c Combo) {
	for _, other := range combos {
		...
		util.Panicif(slices.EqualFunc(c.RCs, other.RCs, func(a, b rowcol.T) bool {
			return a == b
		}))
	}
